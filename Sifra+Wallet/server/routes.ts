...
export async function registerRoutes(
  ...
  app.post("/api/posts/:id/unlock", async (req, res) => {
    try {
      ...
      if (!walletAddress) return res.status(400).json({ message: "Wallet address required" });
      if (!txId) return res.status(400).json({ message: "Transaction ID required. Please complete payment via Kasware." });
      ...
      if (alreadyUnlocked) {
        const contentPayload: any = { success: true, txId };
        if (post.type === "video") {
      ...
      await storage.unlockPost(user.id, post.id, post.price);
      const result: any = { success: true, txId };
      if (post.type === "video") {
  ...
  app.post("/api/posts/:id/unlock-paragraph", async (req, res) => {
    try {
      const { walletAddress, paragraphIndex, txId } = req.body;
      if (!walletAddress) return res.status(400).json({ message: "Wallet address required" });
      ...
      if (alreadyUnlocked.includes(paragraphIndex)) {
        return res.json({ success: true, txId, paragraphContent: paragraphs[paragraphIndex] });
      }

      await storage.unlockParagraph(user.id, post.id, paragraphIndex, post.price);
      res.json({ success: true, txId, paragraphContent: paragraphs[paragraphIndex] });
    } catch (err) {
      res.status(500).json({ message: "Transaction failed" });
    }
  });
